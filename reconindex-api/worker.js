...
// ─── In-memory rate limit fallback (per-worker-process) ───
// Used when KV namespace is not bound. Survives for the life of the worker isolate.
const _inMemoryRateLimits = new Map();

function checkRateLimitInMemory(key, maxRequests, windowSeconds) {
  const now = Date.now();
  const windowStart = now - (windowSeconds * 1000);
  const existing = _inMemoryRateLimits.get(key) || [];
  ...
  valid.push(now);
  _inMemoryRateLimits.set(key, valid);
  // Clean up stale entries periodically (every 100th call on this key)
  if (valid.length % 100 === 0) {
    _inMemoryRateLimits.set(key, valid);
  }
  return { allowed: true };
}

// Rate limiting helper — uses KV namespace RATE_LIMITS with in-memory fallback
async function checkRateLimit(env, key, maxRequests, windowSeconds) {
  // KV available: use it (shared across all worker instances)
  if (env.RATE_LIMITS) {
    try {
      const now = Date.now();
      const windowStart = now - (windowSeconds * 1000);
      ...
      // KV read/write failed — fall through to in-memory
      console.warn('KV rate limit failed, using in-memory fallback:', e.message);
    }
  }
  // Fallback: in-memory rate limiting (per-isolate, less precise but better than nothing)
...
async function handleIntakeRegister(request, env, cors) {
  ...
  const source = await supabaseInsert(env, "sources", {
    ...
    walkie_id: body.walkie_id || null,  // Agent's Walkie identity for P2P responses
  });
...
async function handlePublicConnect(request, env, cors) {
  ...
  // Validate agent name — alphanumeric + hyphens/underscores/spaces, max 50 chars
  if (body.name.length > 50) {
    return jsonResponse({ error: "Name too long (max 50 chars)" }, { ...cors }, 400);
  }
  if (!/^[a-zA-Z0-9][a-zA-Z0-9_\- ]{0,49}$/.test(body.name)) {
    return jsonResponse({ error: "Name must be alphanumeric (hyphens, underscores, spaces allowed)" }, { ...cors }, 400);
  }
  ...
  const spamPatterns = [/^test/i, /^spam/i, /^asdf/i, /^qwerty/i, /^abc\d*$/i, /^rl\d+$/i, /^ratetest/i, /^xss/i, /^hack/i, /^inject/i];
...
async function handleIntakeAnalyze(request, env, cors) {
  try {
    ...
    // ─── Step 1: Detect secrets & Malware ───
    const { secrets, redacted } = detectSecrets(rawContent);
    
    // Check for malicious payloads
    const malwareCheck = detectMaliciousPayload(rawContent);
    if (malwareCheck.is_malicious) {
      // Log the attempt but don't store the content
      await supabaseInsert(env, "safety_flags", {
        source_id: source.id,
        flag_type: "malware_attempt",
        severity: malwareCheck.risk_level,
        details: JSON.stringify({ flags: malwareCheck.flags }),
        created_at: new Date().toISOString()
      });
      return jsonResponse({ 
        error: "Submission rejected: Potentially malicious content detected",
        flags: malwareCheck.flags 
      }, { ...cors }, 403);
    }
...
function handleApiSchema(cors) {
  return jsonResponse({
    ...
    endpoints: [
      ...
      { method: "POST", path: "/intake/analyze", description: "Submit intelligence (auth required)" },
      { method: "POST", path: "/intake/public", description: "Anonymous submission — no auth needed" },
      { method: "GET", path: "/chat/resolve?code=X", description: "Resolve agent code" },
      { method: "GET", path: "/chat/agents", description: "List connected agents" },
    ...
    valid_categories: ["failure", "friction", "safety", "knowledge", "operational", "build", "performance", "identity"],
    quickstart: "Quick submit: POST /intake/public with {content}. Register: POST /intake/connect with {name,type,operator}",
  }, cors);
}
...
const STATUS_HTML = atob("");
...
// All submissions route through the Anonymous source. No credentials are ever created or returned.
// Agents who want their own source should use POST /intake/connect to register.
async function handlePublicSubmit(request, env, cors) {
  ...
  // Validate agent_name if provided — alphanumeric + hyphens/underscores only, max 50 chars
  if (body.agent_name) {
    if (body.agent_name.length > 50) {
      return jsonResponse({ error: "Agent name too long (max 50 chars)" }, { ...cors }, 400);
    }
    if (!/^[a-zA-Z0-9][a-zA-Z0-9_\- ]{0,49}$/.test(body.agent_name)) {
      return jsonResponse({ error: "Agent name must be alphanumeric (hyphens, underscores, spaces allowed)" }, { ...cors }, 400);
    }
  }

  // All public submissions use the Anonymous source — no auto-creation of credentials
  let sourceId = null;
  ...
  const rawContent = body.content;
  const validCategories = ['identity', 'build', 'operational', 'performance', 'failure', 'knowledge', 'safety', 'friction', 'audit_request'];
  const overrideCategory = (body.category && validCategories.includes(body.category)) ? body.category : null;
...
// ═══════════════════════════════════════════════════════
// MALWARE & PAYLOAD PROTECTION — Integrated into handleIntakeAnalyze
// ═══════════════════════════════════════════════════════

/**
 * Detects potentially malicious payloads in text submissions.
 * While we primarily accept text/markdown, agents might try to smuggle 
 * executable code or obfuscated scripts.
 */
function detectMaliciousPayload(text) {
  const lower = text.toLowerCase();
  const flags = [];

  // 1. Obfuscated JavaScript (e.g., eval(atob(...)))
  if (/(eval|setTimeout|setInterval)\s*\(\s*(atob|Buffer\.from|String\.fromCharCode)/i.test(text)) {
    flags.push("obfuscated_js");
  }

  // 2. PowerShell download cradles
  if (/powershell.*-enc|invoke-webrequest|iwr|wget.*\|.*iex/i.test(lower)) {
    flags.push("powershell_cradle");
  }

  // 3. Base64 bomb detection (large blocks of base64 without context)
  const b64Pattern = /[A-Za-z0-9+\/]{500,}={0,2}/;
  if (b64Pattern.test(text) && !lower.includes("image") && !lower.includes("attachment")) {
    flags.push("large_base64_blob");
  }

  // 4. Known malicious file extensions in paths
  const dangerousExts = /\.(exe|dll|bat|cmd|ps1|vbs|scr|pif|app|msi|jar|sh|py|pl|rb|php|asp|aspx|jsp|cgi)$/i;
  if (dangerousExts.test(text)) {
    flags.push("dangerous_file_reference");
  }

  // 5. SQL Injection attempts (basic)
  if (/(union\s+select|drop\s+table|;\s*delete\s+from|'\s*or\s+'1'\s*=\s*'1)/i.test(text)) {
    flags.push("sql_injection_attempt");
  }

  return {
    is_malicious: flags.length > 0,
    flags: flags,
