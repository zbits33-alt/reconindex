export default {
  async fetch(request, env, ctx) {
    ...
    // Landing page for reconindex.com (non-api host, root or common paths)
    const host = request.headers.get("host") || "";
    if (host.includes("reconindex.com") && !host.startsWith("api.")) {
      if (path === "/" || path === "/index.html" || path === "/agent-chat" || path === "/agent-chat.html" ||
          path === "/status" || path === "/status.html" || path === "/suggestions" || path === "/suggestions.html" ||
          path === "/intelligence/xrplpulse") {
        if (path === "/status" || path === "/status.html") return handleStatusPage(request, cors);
    ...
    if (path === "/status" && method === "GET") {
      ...
      if (host2.includes("reconindex.com") && !host2.startsWith("api.")) {
